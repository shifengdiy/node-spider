...
const getMovieDetail = require('./getMovieDetailInfo.js');
...
let reqUrl = 'https://movie.douban.com/j/new_search_subjects?sort=S&range=0,10&tags=%E7%94%B5%E5%BD%B1';
	...
	try {
		...
		setTimeout(function () {
			...
			ax({
				...
				.then(function (res) {
					var cureentPageList = res.data.data;
					if (cureentPageList) {
						index++;
						sumMovieList = sumMovieList.concat(cureentPageList);
						console.log(`开始第${index}次查询...`);
						if (index > maxPage) {
							console.log('全部电影获取完毕，开始写入!');
							let listStream = JSON.stringify(sumMovieList);
							fs.writeFileSync('./static/allMovieList.js', listStream);

							//电影列表获取完毕，开始获取电影详情
							console.log(`电影列表获取完毕，开始获取电影详情`);
							getMovieDetail();
							return;
						}
						getMovieList();
